...
<div class="responsive-container mt-32 w-4/5">
    ...
    <a href="{{ route('survey-reports.create') }}" class="fixed right-12 bottom-12 w-12 h-12 rounded-full text-white text-2xl bg-teal-600 hover:bg-teal-800 flex justify-center items-center">+</a>
    ...
    <div class="grid gap-6 mt-6">
        ...
        <div class="tr border p-4 hover:border-slate-400">
            <a href="{{ route('survey-reports.show', $surveyReport) }}" class="link">{{ $surveyReport->paper->title }}</a>
            <p class="text-sm"> <i class="bi-person"></i> {{ $surveyReport->paper->authors }} | {{ $surveyReport->paper->publication_date }}</p>
        </div>
        @endforeach
    </div>
</div>
