...
<div class="responsive-container mt-32 w-4/5">
    ...
    <a href="{{ route('conference-papers.create') }}" class="fixed right-12 bottom-12 w-12 h-12 rounded-full text-white text-2xl bg-teal-600 hover:bg-teal-800 flex justify-center items-center">+</a>
    ...
    <div class="grid gap-6 mt-6">
        ...
        <div class="tr border p-4 hover:border-slate-400">
            <a href="{{ route('conference-papers.show', $conferencePaper) }}" class="link">{{ $conferencePaper->paper->title }}</a>
            <p class="text-sm"> <i class="bi-person"></i> {{ $conferencePaper->paper->authors }} | {{ $conferencePaper->paper->publication_date }}</p>
        </div>
        @endforeach
    </div>
</div>
